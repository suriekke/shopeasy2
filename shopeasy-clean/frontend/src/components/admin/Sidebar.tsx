...
const Sidebar: React.FC<SidebarProps> = ({ currentPage, onPageChange, isOpen, onToggle }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`
        fixed top-0 left-0 z-30 h-full bg-white shadow-lg transform transition-transform duration-300 ease-in-out
        ${isOpen ? 'translate-x-0' : '-translate-x-48'}
        w-64 lg:translate-x-0 lg:static lg:inset-0
      `}>
        {/* Logo */}
          ...
          <div className="space-y-1">
            {menuItems.map((item) => (
              <button
                ...
                `}
              >
                <span className={`mr-3 ${currentPage === item.id ? 'text-white' : 'text-gray-400'}`}>
                  {item.icon}
                </span>
                {item.name}
              </button>
